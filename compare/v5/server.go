...
func main() {
	...
	mux.HandleFunc("/sync", func(rw http.ResponseWriter, req *http.Request) {
		if err := retry.Do(req.Context(), what, how...); err != nil {
			http.Error(rw, err.Error(), http.StatusInternalServerError)
			return
		}
		rw.WriteHeader(http.StatusOK)
	})
	mux.HandleFunc("/async", func(rw http.ResponseWriter, req *http.Request) {
		if err := retry.DoAsync(req.Context(), what, how...); err != nil {
			http.Error(rw, err.Error(), http.StatusInternalServerError)
			return
		}
		rw.WriteHeader(http.StatusOK)
